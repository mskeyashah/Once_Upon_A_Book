 ...
 */
@WebServlet(name = "CancelReservationServlet")
public class CancelReservationServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        title = title.replace(' ','+');
        	...
        	result = Test.doGet("cancelingbook?title="+title+"&name="+first);
        
        if(result.contains("here is no hold"))
        {
            htmlResponse+= "<p>"+result.substring(2,result.length()-2)+"</p>";
        }
        else
            htmlResponse+= "<p>The book reservation has been canceled.</p>";
